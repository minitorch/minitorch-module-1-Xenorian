...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        ...
        outs = [ self.bias[i].value for i in range(out_size) ]

        # print("outs:",outs)

        for index1,i in enumerate(inputs):
            for index2 in range(out_size):
                outs[index2] += self.weights[index1][index2].value * i
